using LinearAlgebra
# solution of 1D poisson equation u" = d²u/dx² = q(x) on [a,b] valid all over the domain
...
for i ∈ eachindex(X) # loop over each of the N nodes of the node distribution
    ...
    qA[i] = q(xi)                                   # global RHS at center (node i)
                                                    # q(s) expression in u''=q
end

# OK, now we have written our (approximated) differential problem for each node, getting A * uA = qA
# where uA are the values of the field on each point of the domain Ω that I want to find
...
#? Is this the famous q reported on the last equation of the collocation thechnique 
...
qA_internal = qA[internal] - A_RHS*[u_a,u_b] # global RHS for internal nodes - (minus) boundary contribution
                                             #? for the adjoint method we're starting from this system
