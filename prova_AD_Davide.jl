#codice tutto da ricontrollare e rifattorizzare
...
boundary_conditions = [0, 0] #u(X_boundary)   # Decided by us <- occhio che quì le boundary conditions sono dettate dalla funzione q()
...
dJ_du,∂J_∂b = gradient((u_num,b) -> J(u_num,b), u_approximated,b)
...
cᵢ     = zeros(n_points_in_stencil+3)
...
for i in internal_idxs
    ...
    cᵢ .= cᵢ_from_stencil(xi, X[neighbours_idxs], φ, ddφ)

    M   = interpolation_matrix(X[neighbours_idxs], xi, φ)
    α_β = M\[u_approximated_total[neighbours_idxs]; [0,0,0]]
